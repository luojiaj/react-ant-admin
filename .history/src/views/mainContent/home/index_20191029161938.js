...
class Home extends stateData {
  ...
  render() {
    const { selectedRowKeys, tableLoading } = this.state;
    const rowSelection = {
      selectedRowKeys,
    ...
    const columns = [
      ...
      },
      {
        ...
        key: 'isDownloadable',
        render: (text, record) => (
          <Switch checkedChildren="开" unCheckedChildren="关" defaultChecked={record.isDownloadable == 0 ? false : true} />
        ),
      },
      {
        title: '操作',
    ...
    return (
      <div className='home'>
        <Card style={{ 'border-bottom': 0 }} bodyStyle={{ 'height': 62 }}>
          <Button type="danger" className="shanchu" loading={this.state.delLoading}>删除</Button>
        </Card>
        <Card>
          <Table loading={tableLoading} rowSelection={rowSelection} columns={columns} dataSource={this.state.data} />
        </Card>
      </div>
    )
  }
