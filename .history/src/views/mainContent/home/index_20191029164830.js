...
class Home extends stateData {
  ...
  onChange = (record) => {
    ...
    if (record.isDownloadable == 0) user.isDownloadable = 1;
    if (record.isDownloadable == 1) user.isDownloadable = 0;
    putUser(record.id, user).then(res => {
  ...
  render() {
    const { selectedRowKeys, tableLoading, delLoading, data, switchLoading } = this.state;
    const rowSelection = {
      selectedRowKeys,
    ...
    const columns = [
      ...
      },
      {
        ...
        key: 'isDownloadable',
        render: (text, record) => (
          <Switch onChange={() => this.onChange(record)} loading={switchLoading} checkedChildren="开" unCheckedChildren="关" defaultChecked={record.isDownloadable == 0 ? false : true} />
        ),
      },
      {
        title: '操作',
