  ...
  render() {
    ...
    const columns = [
      {
        ...
        render: (text, record) => (
          record.isShowChange == true ?
            <Input ref="nameInput" className="nameInput" onChange={() => this.changeName(record)} defaultValue={record.name} allowClear></Input> :
      ...
      },
      {
        ...
        render: (text, record) => (
          record.isShowChange == true ?
            <Input ref="PhoneInput" className="PhoneInput" onChange={() => this.changePhone(record)} defaultValue={record.phone} allowClear></Input> :
      ...
      },
      {
        ...
        render: (text, record) => (
          record.isShowChange == false ?
            ...
            <span>
              <Button type="danger" onClick={() => this.putChangeUser(record)}>保存</Button>
    ...
    return (
      <div className='home'>
        ...
        <Card>
          <Table ref="table" loading={tableLoading} rowSelection={rowSelection} columns={columns} dataSource={data} rowKey={record => record.id} />
        </Card>
      </div>
    )
  }
