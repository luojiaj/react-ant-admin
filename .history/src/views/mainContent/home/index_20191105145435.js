...
import './index.scss';
import { Table, Card, Button, Switch, message, Input, Divider, Popover, Form, Upload } from 'antd';
import stateData from './data';
import { putAuthority, putDownloadPermission, upLoadFile } from '@/servers/homeApi';
...
class Home extends stateData {
  constructor(props) {
    ...
    this.downloadExcel = this.downloadExcel.bind();
  }
  ...
  downloadExcel = () => {
    window.open(config.url + "/downloadTemplate");
  }
    ...
    return (
      <div className='home'>
        <Card style={{ 'border-bottom': 0 }} bodyStyle={{ 'height': 62 }}>
          ...
          <Button className="download" type="primary" icon="download" onClick={this.downloadExcel}>
            下载模本
          </Button>
          <Upload {...props}>
            <Button className="upload" type="primary" icon="upload">
              上传用户
          </Button>
          </Upload>
